...
interface ChangePasswordDialogProps {
  ...
  hasPassword: boolean
  userEmail: string
  provider: string | null // "google" | "github" | "credentials" | null
}
...
export function ChangePasswordDialog({ 
  ...
  const [verificationMethod, setVerificationMethod] = useState<"password" | "oauth" | null>(null)
  ...
  const handleOAuthMessage = useCallback(async (event: MessageEvent) => {
    ...
    if (event.data.type === "OAUTH_SUCCESS") {
      ...
      if (event.data.user?.email === userEmail) {
        setIsVerified(true)
        setVerificationMethod("oauth")
        setStep("change")
        setIsOAuthLoading(null)
        toast.success("Identity verified!")
      } else {
        // Wrong account - they're now logged in as wrong user!
        // Sign them out and close the dialog
        setIsOAuthLoading(null)
        onOpenChange(false)
        toast.error(
  ...
  const handleOAuthVerify = (provider: string) => {
    ...
    // Open popup with our callback page that will initiate OAuth
    // Pass email as login_hint to pre-select the correct account
    const popup = window.open(
      `/auth/popup-callback?provider=${provider}&email=${encodeURIComponent(userEmail)}`,
      "oauth-popup",
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {step === "verify" && (
          <div className="space-y-4 pt-2">
            {/* Password verification - show if user has password */}
            {hasPassword && (
            ...
            {/* Divider - show when both options are available */}
            ...
            {/* OAuth verification - show based on provider */}
        ...
        {step === "change" && isVerified && (
          <form onSubmit={handleChangePassword} className="space-y-4 pt-2">
            <div className="p-3 bg-green-500/10 border border-green-500/20 rounded-lg">
              <p className="text-sm text-green-600 dark:text-green-400 flex items-center gap-2">
                <Shield className="h-4 w-4" />
                Identity verified via {verificationMethod === "password" ? "password" : "OAuth"}
              </p>
            </div>
